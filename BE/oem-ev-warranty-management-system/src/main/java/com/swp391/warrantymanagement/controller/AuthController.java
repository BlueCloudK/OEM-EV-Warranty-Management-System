...
public class AuthController {
    ...
    public ResponseEntity<Map<String, Object>> createUserByAdmin(@Valid @RequestBody AdminUserCreationDTO adminCreationRequest) {
        logger.info("Admin user creation attempt for username: {}\n", adminCreationRequest.getUsername());
        try {
            ...
            logger.info("Admin user creation successful for username: {}\n", adminCreationRequest.getUsername());
            ...
            logger.error("Admin user creation failed for username: {} - Error: {}\n", adminCreationRequest.getUsername(), e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
            response.put("message", e.getMessage());
            return ResponseEntity.badRequest().body(response);
    ...
    public ResponseEntity<Map<String, Object>> forgotPassword(@Valid @RequestBody ForgotPasswordRequestDTO request) {
        ...
        try {
            ...
            logger.info("Forgot password email sent to: {}\n", request.getEmail());
            return ResponseEntity.ok(Map.of("success", true, "message", "Password reset email sent"));
        } catch (RuntimeException e) {
            logger.error("Forgot password failed for email: {} - Error: {}\n", request.getEmail(), e.getMessage());
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", e.getMessage()));
        }
    }
